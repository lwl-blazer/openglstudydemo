...
//  OpenGLTest_3
//
//  Created by luowailin on 2019/3/4.
...
//地平线
void renderTexturePlane(GLFWwindow *window);
//立方体
void renderTexture3D(GLFWwindow *window);
//更多立方体
...
int main(int argc, char **argv){
    ...
    //renderTexturePlane(window);
    //renderTexture3D(window);
    //renderTextureMore3D(window);
    
    renderCameraRotate(window);
...
void renderTexture3D(GLFWwindow *window){
    /** 深度缓冲(OpenGL存储它的所有深度信息于一个Z缓冲(Z-buffer)中。GLFW会自动为你生成这样一个缓冲)
     深度值存储在每个片段里面（作为片段的z值），当片段想要输出它的颜色时，OpenGL会将它的深度值和z缓冲进行比较，如果当前的片段在其它片段之后，它将会被丢弃，否则将会覆盖。这是深度测试由OpenGL自动完成
     我们要做的就是开启深度测试  glEnable()    / glDisable() 一旦开启这个功能会一直保持启用/禁用状态
